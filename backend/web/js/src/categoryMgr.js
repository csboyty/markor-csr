var categoryMgr=(function(config,functions){
            ...
            "aoColumns": [
                ...
                { "mDataProp": "parent",
                    "fnRender":function(oObj){
                        var string="无";
                        if(oObj.aData.parent){
                            string=oObj.aData.parent.name;
                        }
                        return string;
                    }
                },
                { "mDataProp": "opt",
                    "fnRender":function(oObj){
                        return '<a href="category/update?id='+oObj.aData.id+'">修改</a>&nbsp;' +
    ...
    return {
        ...
        delete:function(id){
            ...
            var me=this;
            $.ajax({
                url:config.ajaxUrls.categoryDelete+"?id="+id,
                type:"post",
                dataType:"json",
                success:function(response){
                    if(response.success){
...
$(document).ready(function(){
    ...
    $("#myTable").on("click","a.delete",function(){
        if(confirm(config.messages.confirmDelete)){
            categoryMgr.delete($(this).attr("href"));
        }
        return false;
    })
});
