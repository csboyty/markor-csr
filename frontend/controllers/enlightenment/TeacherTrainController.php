...
class TeacherTrainController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $activityResults=$activityResultsQuery->
            andWhere(["category_id"=>Yii::$app->params["categories"]["activityTeacherTrain"]])
            ->limit(3)->orderBy(["date"=>SORT_DESC])->all();
        $speechResults=$speechResultsQuery->andWhere(["category_id"=>Yii::$app->params["categories"]["speechTeacherTrain"]])
            ->limit(1)->orderBy(["date"=>SORT_DESC])->all();
        $workResults=$workResultsQuery->andWhere(["category_id"=>Yii::$app->params["categories"]["resultTeacherTrain"]])
